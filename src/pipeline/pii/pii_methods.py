...
def detect_pii_datafog_with_status(text: str) -> Dict:
    """
    Detect PII candidates using DataFog and report whether DataFog failed.

    Important:
    - failed = True means DataFog had a technical/runtime problem.
    - failed = False with empty detections means DataFog executed correctly
      but did not detect valid PII.
    """
    ...
    # If DataFog is unavailable, this is considered a technical failure.
    if (
...
def detect_pii_datafog(text: str) -> List[Dict]:
    ...
    This wrapper preserves the previous function output.
    Use detect_pii_datafog_with_status when the caller needs
    to know whether DataFog failed technically.
    """

    return detect_pii_datafog_with_status(text)["detections"]
...
def detect_pii_combined(text: str) -> List[Dict]:
    ...
    - fallback:
        regex phone/IP detections are used only when DataFog fails
        technically, for example because of a runtime error,
        unavailable module, or model issue.

    Important:
        If DataFog executes correctly and returns no valid phone/IP
        detections, that empty result is considered valid and no
        regex fallback is applied.
    """
    ...
    datafog_result = detect_pii_datafog_with_status(text)
    datafog_detections = datafog_result["detections"]
    datafog_failed = datafog_result["failed"]

    # ========================================================
    # FALLBACK -> ONLY WHEN DATAFOG FAILS TECHNICALLY
    # ========================================================
    ...
    # If DataFog executed correctly but returned no phone/IP
    # detections, do not use regex fallback.
    if not datafog_items_to_confirm:
        return deduplicate_detections(
            final_detections
        )
    ...
    for regex_item in regex_items_to_confirm:
        ...
        for datafog_item in datafog_items_to_confirm:
            ...
            if spans_overlap(
                ...
                matched_by_datafog = True
                break

        if matched_by_datafog:
            final_detections.append(
                regex_item
            )
